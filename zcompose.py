...
"""``zcompose`` - orchestrate multiple Zephyr applications on a laptop.

Conceptually similar to ``docker compose``: a single YAML file describes a
group of applications and the virtual networks they share, and this tool
...
def load_config(path: Path) -> Config:
    ...
    state_dir = compose_dir / ".zcompose" / slug
...
class Compose:
    ...
    def _build_all(self, ctx: Context) -> int:
        ...
        try:
            ...
            self.err(
                ...
                "Install it (pip install tqdm) or use `zcompose build APP` "
    ...
    def _open_console(self, ctx: Context, app_name: str) -> int:
        ...
        if not log.is_file():
            self.err(
                f"no run log for {app_name!r} - is the app running (`zcompose run {app_name}`)?"
            )
            return 1
    ...
    def action_down(self, ctx: Context, *, app: str | None = None) -> int:
        ...
        if alive:
            ...
            self.err(
                ...
                f"Stop them first (Ctrl-C the `zcompose run`) and retry."
            )
            return 1
    ...
    def action_run(self, ctx: Context, *, app: str | None = None) -> int:
        ...
        if missing:
            self.err(
                ...
                f"{', '.join(missing)}. "
                f"Run `zcompose up` first (this requires root "
...
_COMPLETION_BASH = r"""# zcompose bash completion
# Generated by `zcompose completion bash`. Source it in your current shell
# with `source <(zcompose completion bash)`, or install it persistently,
# e.g. `zcompose completion bash | sudo tee /etc/bash_completion.d/zcompose`.

_zcompose() {
    local cur prev cmd file_arg i
    COMPREPLY=()
    cur="${COMP_WORDS[COMP_CWORD]}"
    prev="${COMP_WORDS[COMP_CWORD - 1]}"

    local subcommands="show up down build run clean menuconfig attach-usb console completion"
    local app_subcommands="build run clean menuconfig attach-usb console"
    ...
    if [[ -z "$cmd" ]]; then
        if [[ "$prev" == "-f" || "$prev" == "--file" ]]; then
            ...
            COMPREPLY=($(compgen -W "$subcommands -f --file -p --pristine" -- "$cur"))
        fi
        return
    fi

    case " $app_subcommands " in
        *" $cmd "*)
            if ((COMP_CWORD == i + 1)); then
                local apps
                if [[ -n "$file_arg" ]]; then
                    apps=$(zcompose -f "$file_arg" __list_apps 2>/dev/null)
                else
                    apps=$(zcompose __list_apps 2>/dev/null)
                fi
    ...
    if [[ "$cmd" == "completion" ]] && ((COMP_CWORD == i + 1)); then
        COMPREPLY=($(compgen -W "bash zsh fish" -- "$cur"))
    fi
}

complete -F _zcompose zcompose
"""

_COMPLETION_ZSH = r"""#compdef zcompose

# zcompose zsh completion
# Generated by `zcompose completion zsh`. Install persistently by saving it
# as `_zcompose` in a directory on your $fpath, e.g.
# `zcompose completion zsh > "${fpath[1]}/_zcompose"`.

_zcompose() {
    local cur cmd file_arg i
    cur="${words[CURRENT]}"

    local -a subcommands app_subcommands
    subcommands=(show up down build run clean menuconfig attach-usb console completion)
    app_subcommands=(build run clean menuconfig attach-usb console)
    ...
    case " ${app_subcommands[*]} " in
        *" $cmd "*)
            if ((CURRENT == i + 1)); then
                local -a apps
                if [[ -n "$file_arg" ]]; then
                    apps=(${(f)"$(zcompose -f "$file_arg" __list_apps 2>/dev/null)"})
                else
                    apps=(${(f)"$(zcompose __list_apps 2>/dev/null)"})
                fi
    ...
    if [[ "$cmd" == "completion" ]] && ((CURRENT == i + 1)); then
        local -a shells
...
compdef _zcompose zcompose
"""

_COMPLETION_FISH = r"""# zcompose fish completion
# Generated by `zcompose completion fish`. Install persistently by saving it
# to `~/.config/fish/completions/zcompose.fish` (auto-loaded, no need to
# source it).

function __zcompose_list_apps
    set -l file_arg
    set -l tokens (commandline -opc)
    for i in (seq 2 (count $tokens))
        if test "$tokens[$i]" = -f -o "$tokens[$i]" = --file
            set file_arg $tokens[(math $i + 1)]
        end
    end
    if test -n "$file_arg"
        zcompose -f $file_arg __list_apps 2>/dev/null
    else
        zcompose __list_apps 2>/dev/null
    end
end

complete -c zcompose -f

complete -c zcompose -n "not __fish_seen_subcommand_from show up down build run clean menuconfig attach-usb console completion" -a "show up down build run clean menuconfig attach-usb console completion"
complete -c zcompose -n "not __fish_seen_subcommand_from show up down build run clean menuconfig attach-usb console completion" -s f -l file -r -F -d "compose file"
complete -c zcompose -n "not __fish_seen_subcommand_from show up down build run clean menuconfig attach-usb console completion" -s p -l pristine -d "pristine build"

complete -c zcompose -n "__fish_seen_subcommand_from completion" -a "bash zsh fish"

complete -c zcompose -n "__fish_seen_subcommand_from build run clean menuconfig attach-usb console" -a "(__zcompose_list_apps)"
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    completion_sp = sub.add_parser("completion", help="print a shell completion script")
    completion_sp.add_argument("shell", choices=sorted(_COMPLETION_SCRIPTS))

    # Internal helper used by the generated completion scripts to list the
    # application names declared in the compose file. Not a public command.
    sub.add_parser("__list_apps")

    return parser


def _print_app_names(file_arg: str | None) -> None:
    """Print application names from the compose file, one per line.

    Used internally by the shell completion scripts; never raises, since
    completion must not surface errors in the user's terminal.
    """
    path = Path(file_arg).resolve() if file_arg else Path.cwd() / DEFAULT_FILE
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.action == "completion":
        print(_COMPLETION_SCRIPTS[args.shell])
        return 0
    if args.action == "__list_apps":
        _print_app_names(args.file)
        return 0
    return Compose().run(args)
